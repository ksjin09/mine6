  ...
  *new { arg context, doneCallback;
  ...
  alloc {
    ...
    SynthDef(\effect, {
      ...
      var sig = Array.fill(4,{CombC.ar(In.ar(in,2),0.2,0.01.rrand(0.2))}).sum; //sum 없으면 2차원 배열됨
   10.do({ sig=AllpassN.ar(sig,0.2,0.01.rrand(0.2)) });
	  ...
	  Out.ar(out, LeakDC.ar(sig * fxgain, 0.9));
    }).add;
